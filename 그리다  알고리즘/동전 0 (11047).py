...
# 1. 먼저 동전의 종류 n, ㄱ의 합 k를 입력받아야 한다.
...
# -> reverse 함수를 사용하는 이유는 그리디 알고리즘을 이용하는 과정에서 정렬되어 있는 큰 값을 집어넣었을 때  최솟값을 찾는 루트가 가장 빠르기 때문이다.
...
for i in coin:  
    count += k // i  # 동전의 ㄱ로 나눈 몫을 count에 더합니다. (해당 ㄱ의 동전 개수를 더합니다)
    k %= i  # 나머지를 k에 저장합니다. (해당 ㄱ의 동전을 사용한 후 남은 금액입니다)
